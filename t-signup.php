...
<body>
  ...
  if (isset($_POST['submit'])) {
    ...
    # data insertion
    $checkdouble = "SELECT * FROM tutor WHERE t_Email ='$email'";
    $result = mysqli_query($conn, $checkdouble);
    $present = mysqli_num_rows($result);
    if ($present > 0) {
      $_SESSION['t-email_alert'] = '1';
      // header("Location: learnerLog.php");
    } else {
      $sql = "INSERT INTO tutor(t_Name, t_Email, t_Password)
  ...
  <div class="container">
    <form action="#" method="POST">
      ...
      <span id="error-submit">
        <?php
        $emailmessage = '';
        if (isset($_SESSION['t-email_alert'])) {
    ...
    <?php unset($_SESSION['t-email_alert']); ?>
  </div>
